...
Type SearchContextOfFindStrFunction
  ...
  SheetName As String ' Can be accessed with .Sheet.Name
  SheetIndex As Long ' .Sheet.RangeAddress.Sheet
        ...
        cellvalue = sheet.getCellByPosition(col,row).getString()
            ...
            For Each word In search_context.ContextWords
              If search_context.ContextOptions.SubstringSearch Then
                ' Additional check because of https://ask.libreoffice.org/en/question/71560/calc-macro-case-sensitive-instr-not-working/ '
                temp = InStr(cellvalue,word)
                search_result = IIf(temp > 0, TRUE, FALSE)
                If search_result = TRUE AND search_context.ContextOptions.CaseSensitive Then
                  search_result = IIf(Mid(cellvalue,temp,Len(word)) = word, TRUE, FALSE)
                End If
              Else
